...
**En cours :**
- Notifications push (FCM) — projet Firebase créé (`le-pacte-e6c17`, 3 apps enregistrées : Android `com.kevinarner.le_pacte`, iOS `com.kevinarner.lePacte`, Web). Côté Flutter (28/08) : `firebase_core`/`firebase_messaging` intégrés, `lib/firebase_options.dart` (config des 3 plateformes), `lib/services/notification_service.dart` (permission, token, sauvegarde dans `device_tokens`, appelé depuis `RootShell.initState`), `web/firebase-messaging-sw.js` (service worker web). Schéma `device_tokens` + RLS faits (28/08). Edge Function d'envoi écrite : `supabase/functions/send-notification/index.ts` — reçoit `{ profile_id, title, body, data? }`, s'authentifie auprès de l'API FCM v1 via un compte de service (flux OAuth2 JWT-bearer signé avec `crypto.subtle`, aucune dépendance externe hors `@supabase/supabase-js`), envoie à tous les tokens du profil, supprime automatiquement ceux devenus invalides (`UNREGISTERED`/`NOT_FOUND`). Secret `FCM_SERVICE_ACCOUNT_JSON` ajouté (Dashboard Supabase) et fonction déployée (28/08, via `npx supabase functions deploy`, le CLI Supabase étant indisponible dans ce sandbox). **Chaîne vérifiée de bout en bout en conditions réelles (28/08)** : permission accordée, token obtenu et enregistré dans `device_tokens`, notification envoyée via `curl` et reçue comme vraie notification système macOS/Chrome — après correction de plusieurs bugs (voir section 8 : chemin du service worker, contrainte `plateforme`, conflit avec le service worker interne de Flutter, GRANT manquant sur `device_tokens` pour `service_role`, notifications Chrome désactivées au niveau macOS). Notification reçue app au premier plan : affichée via une bannière in-app (`scaffoldMessengerKey`, `app.dart`/`notification_service.dart`), puisqu'une notification système ne s'affiche pas automatiquement dans ce cas. Reste : les déclencheurs Postgres sur les événements (nouveau pacte, réponse, message, double absence) et le routage au clic sur une notification (`_gererClicNotification` est un point d'accroche, pas encore branché).
