...
- **Refonte UX du hub : "chaque Swend est un objet autonome"** (23/09). Décision produit structurante : la messagerie n'est plus une section indépendante de l'app — `lib/screens/messagerie/messagerie_screen.dart` a été supprimé, il n'y a plus d'écran listant tous les fils de discussion tous pactes confondus. Chaque conversation est désormais rattachée à son pacte : sur `DetailPacteScreen`, `BlocPresence` devient une carte **"Mon relais"** qui affiche le relais désigné pour mon côté (s'il y en a un) avec un accès direct à `ChatScreen` ("Écrire à {prénom}"), ou "Choisir mon relais" sinon ; un lien "Gérer mes relais" reste accessible vers `MesRemplacantsScreen` (renommée "Mes relais" dans l'UI) pour la liste complète. Le mot "remplaçant" est remplacé par "relais" dans cette UI-là spécifiquement — le modèle de données (`Remplacant`, `remplacants`, etc.) n'est pas renommé, seul l'habillage change.
  - Hub (`menu_principal_screen.dart`) restructuré selon la hiérarchie "quel est mon prochain Swend ? est-ce que quelque chose nécessite mon attention ? comment créer/retrouver mes Swends ?" : carte "TON PROCHAIN SWEND", puis une éventuelle carte de notification de message ("{nom} vous a écrit"), puis un bouton `FilledButton` très visible **"+ Créer un Swend"** (nouvel accès direct à `CreerPacteScreen` depuis le hub, en plus du FAB déjà existant sur `AccueilScreen`), puis la tuile "Mes Swends" (`X à venir`, badge si une action est requise), puis "Profil" en simple ligne discrète (plus de tuile-carte). "Nous contacter" n'apparaît plus sur le hub : déplacé dans Profil sous une ligne "Aide / Nous contacter".
  ...
  - Vocabulaire "Pacte" → "Swend" appliqué uniquement là où c'était explicitement demandé (carte du prochain Swend, tuile "Mes Swends", décompte "à venir", et par cohérence directe le titre `AccueilScreen`) — le reste des écrans (titres de détail "Pacte avec X", boutons "Accepter/Refuser le pacte", assistant de création, SMS d'invitation) dit encore "pacte" : renommage volontairement non généralisé à ce stade, à traiter dans une passe dédiée si voulu.
...
- Généraliser le vocabulaire "Swend" aux écrans qui disent encore "pacte" (détail, boutons d'action, assistant de création, SMS).
